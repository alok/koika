...
Module ShiftAddMultiplier (s: Multiplier_sig) <: MultiplierInterface.
  ...
  Definition pair_op_t:=
    {| struct_name := s.pair_n_name;
       struct_fields := [("arg1", bits_t n);
                         ("arg2", bits_t n)
                        ]
    |}.
  ...
  Definition enq : UInternalFunction reg_t empty_ext_fn_t :=
    {{ fun enq (arg: struct_t pair_op_t) : unit_t =>
         let op1 := get(arg, arg1) in
         let op2 := get(arg, arg2) in
         guard (!read0(valid));
         write0(valid, #Ob~1);
         write0(operand1, op1);
         write0(operand2, op2);
...
Module DummyMultiplier (s: Multiplier_sig) <: MultiplierInterface.
  ...
  Definition enq : UInternalFunction reg_t empty_ext_fn_t :=
    {{ fun enq (arg: struct_t pair_op_t) : unit_t => pass }}.
