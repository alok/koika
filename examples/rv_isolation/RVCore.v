...
(* The Core is a 4-stage (Fetch, Decode, Execute, Writeback) core implementing RV32I.
   It is extended with a custom Enclave instruction (R-type) for context switching:
     - opcode: Ob~0~0~0~1~0~1~1
     - funct3: Ob~0~0~0
     - funct7, rd, rs2: all zeroes
     - rs1: register containing the enclave configuration to switch, with
       + v[0:2] := EnclaveId  (0->4)
       + v[2:8] := SharedId   (locked mailbox regions in config)
       + v[8]   := UART       (1 if config can access the MMIO UART )
       + v[9]   := LED        (1 if config can access the MMIO LED )
       + v[10]  := Finish      (1 if config can access the MMIO FINISH )
   It contains a purge rule that purges microarchitectural state, and a purge state machine that freezes execution when purged.
 *)
...
Module RV32Core.
          ...
          if get(dInst, legal) == Ob~0 then
            ...
            (let fInst := get(dInst, inst) in
             ...
             if isMemoryInst(dInst) then
               ...
               (set enclaveData := rs1_val; (* Enclave context switch: freeze the pipeline *)
  ...
  Definition rl_purge : uaction reg_t ext_fn_t :=
    {{ guard (read0(purge) == enum purge_state { Purging });
       ...
       (* Testing out not completely resetting the FIFOs *)
       f2d.(fromFetch.reset)();
...
Module Core <: Core_sig.
  ...
  (* Ordering of the rules allows the pipeline stages to run in the same cycle. *)
  Definition schedule : scheduler :=
    Writeback |> Execute |> StepMultiplier |> Decode |> WaitImem |> Fetch |>  Tick |> Init |> Purge |> done.
