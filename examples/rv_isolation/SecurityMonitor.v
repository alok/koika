(*! Implementation of a security monitor *)
...
| Rl_UpdateEnclave (core: ind_core_id) (* Update enclave configurations *)
| Rl_FilterReqs (core: ind_core_id) (* Filter reqs from core -> mem *)
| Rl_ForwardResps (core: ind_core_id) (* Forward resps from mem -> core *)
| Rl_ExitEnclave (core: ind_core_id) (* Exit enclave if core and memory have purged, resetting state *)
| Rl_EnterEnclave (core: ind_core_id) (* Enter an enclave if allowed to by the enclave model *)
| Rl_DoMMIO (* Arbitrate access to shared MMIO *)
| Rl_DoClk.

Inductive sm_reg_t :=
| toMMIO0 (state: MemReqBypass.reg_t) (* FIFOs to/from MMIO per core *)
...
Module SM (Params: EnclaveParams_sig) <: SM_sig.
  ...
  (* Filter out-of-region memory requests *)
    ...
    {{ if (fromCore.(can_deq)() && toMem.(can_enq)()) then
         ...
         let enc_data_and_addr :=
           struct enclave_req_and_addr { req := get(enc_data, data);
                                         addr := address } in
         if (filter_fn (enc_data_and_addr) && enc_valid) then
           toMem.(enq)(request)
         else pass
       else pass
    }}.
  ...
  (* Check if address is in the EnclaveId range *)
  ...
  (* Check if address is in a shared region owned by the configuration *)
  ...
  (* An address is valid for the configuration if it is in the enclave region or locked mailbox region *)
  ...
  (* Filters addresses not in range, for the IMem, DMem, and MMIO *)
  ...
  (* Arbitrate shared access to the MMIO bus using the clk *)
  ...
  (* Handle requests to switch enclaves *)
  ...
  Definition reset_fifos (core: ind_core_id): uaction reg_t ext_fn_t :=
    ...
    {{
        fromCoreImem.(MemReqBypass.reset)();
        fromCoreDmem.(MemReqBypass.reset)();
        fromCoreMMIO.(MemReqBypass.reset)();
        fromCoreEnclave.(Core.toSMEnc.reset)();
        fromMemImem.(MemRespBypass.reset)();
        fromMemDmem.(MemRespBypass.reset)();
        fromMemMMIO.(MemRespBypass.reset)()
    }}.
