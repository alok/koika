...
int main0 () {
  ...
  putchar0('S');
  putchar0('@');
  putchar0(secret_code);
//  putchar0('\n');
//  blink('.');
//  blink('-');
  // Switch to another enclave and add password
  _add_password(1, 'S', secret_code);
  // Switch to other enclave and lookup password
  char ch = _lookup(1, secret_code);
  // Output the result
  putchar0(ch);
//  blink('.');
  // Lookup with the wrong secret
  ch = _lookup(1, 0);
  // Expect 0
  putchar0(ch);

  _add_password(1, 'T', secret_code);
  ch = _lookup(1, secret_code);
  putchar0(ch);
