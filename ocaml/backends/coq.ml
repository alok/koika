...
let pp_int_fn ~print_positions ppf (_, { uint_name; uint_argspec; uint_retType; uint_body }) =
  ...
  p "int_argspec := %a;@ " (pp_list (pp_pair pp_quoted pp_type_unwrapped)) uint_argspec;
  p "int_retType := %a;@ " pp_type_unwrapped uint_retType;
  p "int_body := %a;" (pp_action print_positions) uint_body;
  p "@]@ |}.@]"
